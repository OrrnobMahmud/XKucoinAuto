...
class BlumBot:
    def __init__(self, file_path="data.txt"):
        ...
        self.encoded_data_list = self.read_data_file()
    ...
    ____  _             _       _        _
   | __ )| | ___   __ _(_)_ __ (_)_ __  (_)_ __
   |  _ \| |/ _ \ / _` | | '_ \| | '_ \ | | '_ \
   | |_) | | (_) | (_| | | | | | | |_) || | | | |
   |____/|_|\___/ \__, |_|_| |_|_| .__/ |_|_| |_|
                 |___/           |_|

        Welcome to Blum Auto - KuCoin Bot
...
-----------------------------------------------
    ...
    def tap(self, cookie, increment, molecule):
        ...
        for _ in range(15):
            ...
            random_color = random.choice([Fore.GREEN, Fore.YELLOW, Fore.BLUE, Fore.MAGENTA, Fore.CYAN])
            print(f"{random_color}{Style.BRIGHT}Tapped{Style.RESET_ALL}")
            time.sleep(2)

    def new_balance(self, cookie):
        url = "https://www.kucoin.com/_api/xkucoin/platform-telebot/game/summary?lang=en_US"
        headers = self.build_headers(cookie=cookie)
        response = requests.get(url, headers=headers)
        data = response.json()
    ...
    def main(self):
        while True:
            ...
            for index, encoded_data in enumerate(self.encoded_data_list, start=1):
                print(f"{Fore.CYAN + Style.BRIGHT}------Account No.{index}------")
                decoded_data = self.decode_data(encoded_data)
                cookie = self.login(decoded_data)
                molecule = self.data(cookie)
                increment = random.randint(40, 60)
                self.tap(cookie, increment, molecule)
                self.new_balance(cookie)
                self.countdown_timer(2)

            self.countdown_timer(1 * 2 * 60)
